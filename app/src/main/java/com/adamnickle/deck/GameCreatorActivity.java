...
public class GameCreatorActivity extends PreferenceActivity implements Preference.OnPreferenceChangeListener
{
    ...
    private static final String KEY_PREF_CARDS_SPADES_CATEGORY = "pref_cards_spades_category";
    private static final String KEY_PREF_CARDS_HEARTS_CATEGORY = "pref_cards_hearts_category";
    private static final String KEY_PREF_CARDS_CLUBS_CATEGORY = "pref_cards_clubs_category";
    private static final String KEY_PREF_CARDS_DIAMONDS_CATEGORY = "pref_cards_diamonds_category";
    ...
    {
        PreferenceScreen customPointsValueScreen = (PreferenceScreen) mGameCreatorFragment.findPreference( KEY_PREF_CUSTOM_POINTS_STYLE_SCREEN );
        PreferenceCategory category;
        EditTextPreference pref;

        category = (PreferenceCategory) mGameCreatorFragment.findPreference( KEY_PREF_SUITS_CATEGORY );
        ...
        category = (PreferenceCategory) mGameCreatorFragment.findPreference( KEY_PREF_RANKS_CATEGORY );
        ...
        category = (PreferenceCategory) mGameCreatorFragment.findPreference( KEY_PREF_CARDS_CATEGORY );
        PreferenceScreen spades = (PreferenceScreen) mGameCreatorFragment.findPreference( KEY_PREF_CARDS_SPADES_CATEGORY );
        PreferenceScreen hearts = (PreferenceScreen) mGameCreatorFragment.findPreference( KEY_PREF_CARDS_HEARTS_CATEGORY );
        PreferenceScreen clubs = (PreferenceScreen) mGameCreatorFragment.findPreference( KEY_PREF_CARDS_CLUBS_CATEGORY );
        PreferenceScreen diamonds = (PreferenceScreen) mGameCreatorFragment.findPreference( KEY_PREF_CARDS_DIAMONDS_CATEGORY );
        Card card;
        ...
        setPreferenceChangeListener( mGameCreatorFragment.getPreferenceScreen(), this );
        initializePreferences( mGameCreatorFragment.getPreferenceScreen(), this );
    }
    ...
    private void setPreferenceChangeListener( PreferenceGroup preferenceGroup, Preference.OnPreferenceChangeListener preferenceChangeListener )
    {
        ...
        for( int i = 0; i < count; i++ )
        {
            ...
            {
                setPreferenceChangeListener( (PreferenceGroup) pref, preferenceChangeListener );
            }
        }
    }
    ...
    private void initializePreferences( PreferenceGroup preferenceGroup, GameCreatorActivity gameCreatorActivity )
    {
        ...
        for( int i = 0; i < preferenceGroup.getPreferenceCount(); i++ )
        {
            ...
            if( pref instanceof PreferenceGroup )
            {
                initializePreferences( (PreferenceGroup)pref, gameCreatorActivity );
            }
            else if( pref instanceof ListPreference )
            {
                value = ( (ListPreference) pref ).getValue();
                gameCreatorActivity.onPreferenceChange( pref, value );
            }
            ...
            {
                boolean bool = ( (CheckBoxPreference) pref ).isChecked();
                gameCreatorActivity.onPreferenceChange( pref, bool );
            }
        }
    }
    ...
    private void returnResult()
    {
        ...
        Game game = new Game();
        game.GameName = ( (EditTextPreference) mGameCreatorFragment.findPreference( KEY_PREF_GAME_NAME ) ).getText();
        game.MinimumPlayers = Integer.parseInt( ( (EditTextPreference) mGameCreatorFragment.findPreference( KEY_PREF_MIN_PLAYERS ) ).getText() );
        game.MaximumPlayers = Integer.parseInt( ( (EditTextPreference) mGameCreatorFragment.findPreference( KEY_PREF_MAX_PLAYERS ) ).getText() );
        game.TrackPoints = ( (CheckBoxPreference) mGameCreatorFragment.findPreference( KEY_PREF_TRACK_POINTS ) ).isChecked();
        if( game.TrackPoints )
        {
        ...
        game.DealFullDeck = ( (CheckBoxPreference) mGameCreatorFragment.findPreference( KEY_PREF_DEAL_FULL_DECK ) ).isChecked();
        if( !game.DealFullDeck )
        {
            game.InitialCardsPerPlayer = Integer.parseInt( ( (EditTextPreference) mGameCreatorFragment.findPreference( KEY_PREF_INITIAL_CARDS_PER_PLAYER ) ).getText() );
        }
    ...
    private int getIdentifierForPointStyle( String style )
    {
        ...
        return getResources().getIdentifier( identifierName, "array", getPackageName() );
    }

    private int[] getCardPointValues()
    {
        int values[] = null;
        ListPreference pointValueStylePreference = (ListPreference) mGameCreatorFragment.findPreference( KEY_PREF_POINTS_VALUE_STYLE );
        String pointStyle = pointValueStylePreference.getValue();
        if( pointStyle.equals( "Custom" ) )
        {
            values = new int[ Deck.CARD_COUNT ];
            ListPreference cardPointsSetMethodPreference = (ListPreference) mGameCreatorFragment.findPreference( KEY_PREF_CARD_POINTS_SET_METHOD );
            ...
            if( setMethod.equals( "By Suit" ) )
            {
                for( int i = 0; i < Deck.SUITS; i++ )
                {
                    ...
                    pref = (EditTextPreference) mGameCreatorFragment.findPreference( key );
    ...
    public static class GameCreatorFragment extends PreferenceFragment
    {
        ...
        public void onCreate( Bundle savedInstanceState )
        {
            ...
            gameCreatorActivity.onFragmentCreated();
        }
    }
}
